function lin_model = transform_2p1t_to_milp_ccwi2023(...
      input, linprog, network, pump_groups, init_sim_output, ...
      save_to_mps, save_to_mat, var_names, forced_final_tank_level, ...
    ...
    % Set the vectcor of objective function coefficients c
    ...
    [lb_vector, ub_vector] = set_variable_bounds(vars, constraints, forced_final_tank_level);
    ...
    if save_to_mps == true
        ...
        OK = SaveMPS(mps_filename, mps_text);
        % '../python/data/2p1t/2p1t.mps'
    end
    % Save to a mat file if save_to_map is True
    if save_to_mat == true
        % '../python/data/2p1t/2p1t_model.mat'
        save(mat_filename, '-struct', 'lin_model');
    end
