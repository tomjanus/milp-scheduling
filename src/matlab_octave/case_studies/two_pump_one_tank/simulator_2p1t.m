function output = simulator_2p1t(ht0,K,schedule,network,tank, input,pump, time_step)
  ...
  for k=1:K
    ...
    x=fsolve(@(x)hydraulics_2p1t(x, n, s, dk, input.hr, htt, ...
      network, pump), x0);
    ...
    P(k) = pump_power_consumption(pump, q(network.elements.pump_pipe_index,k), s, n) * ...
      input.tariff(k);
    % Advance state in time
    % Calculate water of volume passing through the tank feed pipe
    dV = q(network.elements.tank_feed_pipe_index,k)*time_step;
